...
│   ├── (shop)/             # Customer storefront (home, products, cart, checkout, account, feedback, track — G18 guest order status: /track form + /track/[orderNumber] page)
...
│   │   ├── orders/         # Customer order endpoints; orders/lookup is PUBLIC by design (G18) — the order-number + e-mail pair is the credential
...
│   ├── legal.ts            # Seller-identity config for the legal pages (G23): `LegalEntity` type, null-gated `LEGAL_ENTITY` (CLIENT-SUPPLIED-pending — the 2026-09-16 delegation gave us the COPY, not the FACTS; `null` is a supported production state, so <SellerRequisites/> renders the brand+contact fallback and /terms, /privacy, /returns publish without a ЄДРПОУ rather than with a fabricated one), and `RETURN_WINDOW_DAYS = 14` (USER-APPROVED 2026-07-28). Config only — no copy, no JSX
...
│   ├── order-access.ts     # Guest order access rule (G18): canAccessOrder() = owning session → valid signed per-order `og_<N>` grant cookie → else redirect; HMAC-SHA256 grants keyed with NEXTAUTH_SECRET (24 h), order-number gate (pattern + 40-char cap shared with the lookup schema), lockout constants
...
│   │   ├── order-confirmation.ts     # Order confirmation template (OrderEmailData; CTA: account → /account/orders, guest → /track?order=N since G18)
...
│   ├── order-access.test.ts          # G18: grant sign/verify, order-number gate (incl. generator ↔ pattern contract), canAccessOrder
│   ├── orders-lookup-api.test.ts     # G18: POST /api/orders/lookup — uniform 404, lockout semantics (count-before-compare, counter kept under lock, lazy reset), grant cookie
│   ├── track-form.test.tsx           # G18: /track form byCode mapping + navigation
│   └── robots.test.ts                # robots.txt disallow list (incl. /track/)
...
├── ru.json                 # agent-translated 2026-08-14, ACCEPTED 2026-09-16 by client delegation (TASK-056 item 20; «Нова Пошта» undeclined, «ё» kept) — deep-merges over uk.json at request time; a missing/malformed key silently falls back to the UA value
...
**Auth flow**: NextAuth v5 with JWT strategy. Middleware protects `/account` (auth required) and `/admin` (ADMIN role required); `/checkout` is deliberately public since G2 (guest COD checkout — orders link to the account when a session exists). API routes use `requireAdmin()` / `requireAuth()` guards from `api-utils.ts`. **Order pages are grant-or-session gated since G18**: `/checkout/confirmation?order=N` and `/track/[orderNumber]` render only for the owning session or a holder of a valid signed per-order `og_<N>` cookie (`src/lib/order-access.ts`, set by `create-order` and by a successful `POST /api/orders/lookup`); absent AND unauthorized orders redirect identically to `/track?order=N`, so the pages are not an existence oracle.
...
- **Coded API outcomes**: newsletter routes (`api/newsletter/{subscribe,confirm,unsubscribe}`), the register 409 (`api/auth/register`), `api/feedback` (`FEEDBACK_SENT`/`VALIDATION_ERROR`/`SEND_FAILED`, G8), and `api/orders/lookup` (`ORDER_NOT_FOUND`/`TOO_MANY_ATTEMPTS` with `retryAfterSeconds`/`VALIDATION_ERROR`/`LOOKUP_FAILED`, G18 → `track.byCode`) return a machine `code` alongside English prose; clients map `code` → locale copy via each namespace's own `byCode` object in `messages/{uk,ru}.json` (`newsletter.confirm.byCode`, `newsletter.unsubscribe.byCode`, `newsletter.signup.byCode`, `feedback.byCode`), guarded by next-intl's `t.has(key as never)` before rendering a dynamic key, with a namespace `fallback` string when the code has no catalog entry (TASK-039 G9 superseded the G2 `create-order`/G4/G8-era `src/content/{newsletter,feedback}.ts` byCode maps — both files deleted)
- **Malformed request bodies answer 400, never 5xx** (G20, PR #46): every public route that reads a JSON body parses it as `await request.json().catch(() => null)` — the idiom G18 introduced in `api/orders/lookup` — and lets `null` fall into the validation the route already runs, so no route carries a branch for it. Routes on `safeParse` return their existing `VALIDATION_ERROR` 400; routes on throwing `.parse()` raise a `ZodError` their pre-existing catch already narrows to 400. Without it `request.json()` throws a `SyntaxError` into the generic catch and a client's bad request is reported as **our** server failure — a status that tells a non-browser caller to retry something that can never succeed. **All 10 public JSON routes carry it, with no exclusions**: the dormant Stripe pair (`create-payment-intent`, `confirm-order`) is included precisely so there is no special case to drift (PR #46 review — excluding one dormant route while fixing its equally dormant sibling was an uneven rule, not a reason). `api/admin/**` is deliberately out of scope (an authenticated administrator is the only reachable caller). Enforced by `tests/unit/malformed-json-body.test.ts`, which pairs behavioral tests with a source enumeration that **fails when a new public route lands without the idiom** — plus a tie-in assertion making an enumerated-but-untested route a failure in itself, so the enumeration cannot outgrow its behavioral coverage
- **Guest order access** (G18, PR #44): one helper owns the rule (`canAccessOrder` in `src/lib/order-access.ts`) and both order pages call it; the grant is a signed per-order httpOnly cookie (`og_<N>` = `<expiry>.<HMAC-SHA256>`, `Secure` iff the request is https, `SameSite=Lax`, 24 h), never a token in the URL. `POST /api/orders/lookup` (public) verifies the order-number + e-mail pair and sets the grant; lockout is per order (two additive `Order` columns): the attempt is counted **before** comparing, the lock decision reads the returned count, a lock write **keeps** the counter (zeroing it let in-flight siblings compare — PR #44 review), and the counter resets lazily when an expired lock is observed, so a burst gets at most five comparisons per 15-minute window; 429 carries `Retry-After`. Unknown number and wrong e-mail answer the identical 404; the timing/side-effect difference is a recorded residual. The order-number gate (`ORDER_NUMBER_PATTERN` + `ORDER_NUMBER_MAX_LENGTH`) is shared by the schema and the page gate, and a unit test asserts `generateOrderNumber()` satisfies it — change the generator and the gate together. No per-IP throttle (prod has no Redis; WAF rule BACKLOG'd)
- **Transactional email pattern** (G5): all emails (order confirmation, newsletter double opt-in, feedback notification — the last added in G8) render on the shared dark Mirox table-based shell (`src/lib/email-templates/layout.ts` — `renderEmailShell`/`renderPanel`/`renderButton`, `lang="uk"`, bgcolor attrs for Outlook); UA copy sourced from `src/content/emails.ts`, never inlined; brand name resolved at render time via `getStoreName()` (`env NEXT_PUBLIC_STORE_NAME || BRAND_NAME`) — a module-scope const would freeze the env value at import and break env-dependent tests; every interpolated free-text user/DB string passes through `escapeHtml` (item names, variant info, address fields, subscriber email); order CTA is guest-aware (`OrderEmailData.hasAccount`) — signed-in customers get «ІСТОРІЯ ЗАМОВЛЕНЬ» → `/account/orders`, guests get «СТАТУС ЗАМОВЛЕННЯ» → `/track?order=N` (G18 superseded the G2 ruling that hid the button for guests, since guests now have a page to land on); tax row renders only when `tax > 0` (COD is always 0); contact links pull from `brand.ts` `SOCIALS`/`WHATSAPP_HREF`, so WhatsApp stays hidden until the client supplies a real number; **route sends are `await`ed** — an unawaited fire-and-forget dies when the serverless function freezes after responding, so prod never sent (regression-tested by the create-order race test)
- **Cookie-mode i18n via next-intl** (TASK-039): UA is the default and SEO-canonical locale; RU is a pure client-preference toggle stored in a `NEXT_LOCALE` cookie (`src/i18n/config.ts` — `LOCALES`, `DEFAULT_LOCALE`, `resolveLocale()` never trusts the wire value). No URL routing — every route keeps the same path in both locales, deliberately (a toggle, not a routing migration). `src/i18n/request.ts` (`getRequestConfig`) resolves the cookie server-side and deep-merges `messages/ru.json` over `messages/uk.json` (`src/i18n/merge.ts`'s `deepMerge`), so a missing or shape-mismatched RU key silently falls back to the UA value — partial RU coverage never breaks the UI. `LocaleSwitcher` (`src/components/common/LocaleSwitcher.tsx`, mounted in Header) calls the `setLocale` server action (`src/i18n/actions.ts`), which sets the cookie and lets router-cache invalidation re-render the tree in the new locale. **Component rule**: non-async components/functions call `useTranslations(namespace)`; async functions (SEO metadata helpers, any Server Component that awaits before rendering) call `await getTranslations(namespace)` instead — `src/lib/seo.ts`'s 6 metadata helpers (`getDefaultMetadata`, `getProductMetadata`, `getHomeMetadata`, `getProductsListingMetadata`, `getCategoriesListingMetadata`, `getAuthMetadata`) and the root `src/app/layout.tsx`'s `generateMetadata()` are all async and use `getTranslations` (the one former sync holdout, `getCategoryMetadata`, was deleted with the `/categories/[slug]` page in G12). **byCode guard pattern**: `t.has(key as never) ? t(key as never) : t("fallback")` before rendering a dynamic coded-outcome key (next-intl v4.13.6) — see the Coded API outcomes pattern above. **Verification**: `scripts/i18n-byte-diff.mjs` diffs `src/**/*.{ts,tsx}` against `main`, extracts every Cyrillic string-literal fragment on a removed line, and fails if it isn't present verbatim in `messages/uk.json` — catches transcription corruption (the «цінує»→«цінює» class); deliberate rewrites (e.g. UA dev comments translated to English) are allowlisted one fragment per line in `scripts/i18n-byte-diff-allow.txt`. **Tests**: `tests/helpers/render-with-intl.tsx`'s `renderWithIntl()` wraps RTL renders in `NextIntlClientProvider` with the real `uk.json` messages, so component tests assert against actual catalog strings, not mocks. **Admin namespace shipped (G13)**: `admin.*` is populated and catalog-driven — admin chrome (sidebar, page titles, forms, tables, dialogs, toasts) renders in Ukrainian; status badges (orders, products, reviews, suppliers) source their labels from the catalog instead of raw enum values (`account.orderStatus`/`account.paymentStatus` reused for order/payment badges; supplier order status via `admin.supplierOrderStatus`). **UA-only by decision** (2026-08-16): RU deliberately has no `admin.*` keys — the deep-merge fallback renders UA for RU-toggled admins, since admin is an internal tool, not customer-facing. **Provider split**: the root `src/app/layout.tsx` strips a named exclusion list of namespaces out of the messages object before handing it to the storefront's `NextIntlClientProvider`; the `(admin)` route group's own layout re-provides the full unfiltered catalog via a nested `NextIntlClientProvider`. The list is `STOREFRONT_EXCLUDED_NAMESPACES` in `src/i18n/client-namespaces.ts` — a deliberately dependency-free leaf module, because `layout.tsx` imports `next/font/google` and `globals.css` and so cannot be imported by a unit test. It holds **`admin`** (G13 — admin chrome must never ship to the public bundle) and **`pages`** (G23 — the seven info/legal routes are Server Components, so their long-form Cyrillic prose has no client consumer and would otherwise ride the RSC payload of _every_ storefront page — **47,787 bytes**, measured 2026-09-22 as the difference between the catalog minus `admin` alone (76,855) and minus both (29,068)). Enforced by `tests/unit/client-messages-payload.test.ts`, which asserts the list's exact contents and compares the surviving namespaces against the catalog's own key set rather than `length > 0`. **Adding a namespace to that list means no client component may call `useTranslations()` on it** — Server Components still read the full catalog through `getTranslations()` regardless. **`pages` namespace (G23)**: `pages.{terms,privacy,returns,faq,shipping,about,contact}` plus `pages.common.requisites`; six of the seven render through the shared async `StaticPage` shell (`src/components/pages/StaticPage.tsx`), which reads `sections` via `t.raw()` — `t.raw` performs no ICU interpolation, which is why `RETURN_WINDOW_DAYS` cannot be threaded through the copy and «14 днів» is written literally (tied back to the constant by `tests/unit/static-pages.test.tsx`). Like `admin`, `pages` is **UA-only by decision** — no `ru.json` keys, deep-merge renders UA. One deliberate residual: `ProductForm.tsx` keeps three literal «грн» currency-unit suffixes next to price inputs — a currency symbol, not translatable copy, so it stays hardcoded regardless of locale. The former `content/account.ts` label duplicate was deleted in PR #37 review round 2 — it had no production consumers.
...
- **Active tasks**: v1.3 "Mirox Rebrand Demo" — TASK-034/035/036/037/057 shipped (PRs #19, #21/#23, #26, #27, #24); **G18 guest order access shipped 2026-09-06** (PR [#44](https://github.com/GoodAlex223/dropshipping-test/pull/44) merged `a37c8d0` — closes the G17 MEDIUM confirmation-page PII disclosure and adds `/track`); **TASK-039 i18n shipped 2026-08-15** (G9, PR #37 `2c93da7` — next-intl cookie mode, UA default + RU toggle; the RU catalog was accepted 2026-09-16 under the client's TASK-056 blanket delegation), **G13 admin-panel translation shipped 2026-08-17** (PR [#40](https://github.com/GoodAlex223/dropshipping-test/pull/40) merged `56328f0` — dashboard + all admin routes catalog-driven UA; the former EN/raw-enum admin chrome is gone; see the Admin namespace note above), **TASK-058/059 shipped 2026-08-14** (G8, PRs #35/#36 — feedback form + launch marquee); remaining v1.3: TASK-040 CI extensions, plus spawned TASK-055/056 (content/legal, client inventory) — see docs/superpowers/specs/2026-07-14-mirox-shop-program-design.md for the Mirox Shop program (rebrand + Ukraine launch, milestones v1.3/v1.4/v2.0)
