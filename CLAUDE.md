...
├── content/                # Typed content-CONFIG layer only (TASK-039 G9 externalized all display copy to messages/*.json; cart.ts/auth.ts/feedback.ts/newsletter.ts/system.ts deleted outright — their copy lives in the catalog now — the rest trimmed to non-translatable config)
│   ├── account.ts          # ORDER_STATUS_LABELS/PAYMENT_STATUS_LABELS only — temporary admin-only duplicate of messages/uk.json's account.orderStatus/paymentStatus, deleted once G13 migrates admin to the admin.* namespace
