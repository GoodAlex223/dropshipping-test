...
- 🟤 **The `?sort=newest` deep links are inert until TASK-036** — the hero secondary CTA and both rails' "View all" point at `/products?sort=newest`, but `/products` ignores the `sort` param until TASK-036 implements sorting. Harmless (lands on the catalog) but the intent silently doesn't apply. Verify these links resolve to real sorted results when TASK-036 ships. (Low value, Low effort) `[relates-to: TASK-036]`
- 🟤 **Product prices render in USD while the rebrand copy is Ukraine/UAH-facing** — the homepage rails show `ProductCard` prices formatted in USD, but the brand copy, delivery language and launch target are Ukraine (₴). This will read as inconsistent until the UAH work lands. Reconcile as part of TASK-039 (UAH formatting per decision doc §7.4). (Med value, — effort tracked under TASK-039) `[relates-to: TASK-039]`
...
- 🟤 **The live catalog is the old generic electronics seed with dead image URLs, not Mirox clothing** — every homepage product card on prod renders a **blank gray image** (verified: `product.images[0].url` is set — otherwise `ProductCard`'s `Package` fallback icon would show — but the URL 404s on prod), and the products themselves are _Minimalist Desk Lamp / Smart Watch Pro / Wireless Bluetooth Headphones_ etc. — the pre-rebrand seed data, unrelated to a clothing brand. For a clothing store this is the single most damaging on-screen fact: no garment photography and an off-brand catalog. Distinct from TASK-056 (which enumerates hero photo, logo, socials, legal copy but **not** a real product catalog + working product imagery) and from the stale-seed-count entry (which is about doc drift, not the live data). Needs real Mirox products imported with resolvable image assets (S3/CDN reachable from prod). (High value, Med effort) `[relates-to: TASK-056; TASK-036]`
...
- 🟤 **TASK-043 (cart, v1.4) — design now exists**: `docs/design/design_handoff_mirox/Mirox Cart.dc.html` specifies quantity steppers, a sticky order summary, a promo-code field, and a dashed-border empty-cart state. Not built by TASK-057 (the cart page only got the dark-coherence colour sweep, Task 11) — recorded here as a pointer for whenever TASK-043 is promoted to TODO.md/v1.4. (Low value, Low effort) `[relates-to: TASK-043]`
- 🟤 **TASK-048/049 (payments/Nova Poshta, v1.4) — checkout design now exists**: `docs/design/design_handoff_mirox/Mirox Checkout.dc.html` specifies a 3-step flow, Nova Poshta radio-cards (відділення 80 грн / кур'єр 120 грн / поштомат 70 грн — published rates, negotiable; real economics need sales quotes per the payments decision doc), and a payment-method step («Карткою онлайн» vs «Оплата при отриманні», with the COD path mapping to `AWAITING_COD` per decision doc §8.4). **Known interim mismatch, unresolved until these tasks land**: `SHIPPING_METHODS` in `src/lib/stripe.ts` still carries its original 5.99/12.99/24.99 USD values, which `formatPrice()` now renders as if they were грн (e.g. "5.99 грн" instead of a real Nova Poshta rate) — the shipping-method/currency swap belongs to TASK-048/049, not TASK-057. Recorded here as a pointer for whenever these are promoted to TODO.md/v1.4. (Low value, Low effort) `[relates-to: TASK-048; TASK-049]`
- 🟤 **Header «Бестселери» is admin-curated, not sales-ranked** — `src/components/common/Header.tsx`'s nav links «Бестселери» to `/products?featured=true`, i.e. whatever an admin flags `isFeatured` on, not actual sales performance. `src/lib/product-queries.ts`'s `getBestsellers()` already computes a real sales-ranked list (with backfill) but has no consumer since TASK-057 (see the entry above). The nav label is honest-enough copy today only because "Бестселери" reads as "our picks" in common usage, but it stops being accurate once TASK-036 ships a real popularity sort — wire the nav (or the catalog's default sort for that link) to the real ranking then. (Med value, Low effort) `[relates-to: TASK-036]`
- 🟤 **Post-deploy verification owed: fetch a real PDP `og:image` from prod** — both OG routes (`src/app/opengraph-image.tsx` and `.../products/[slug]/opengraph-image.tsx`) now read `public/images/og-logo-ghost.png` via `fs.readFileSync` at module load (TASK-057 gate-revision fix) instead of an absolute-URL fetch. The root route is build-time-safe: it's statically prerendered once, so if the file weren't traced into the build, the build itself would have failed loudly (it didn't — verified by rendering the built output). The PDP route is different: it's dynamic (rendered per request), so a tracing gap there would 500 on every live request instead of failing the build — something only a real prod request can prove, since local dev/build always has full filesystem access regardless of Vercel's `@vercel/nft` trace. Action: after the next prod deploy, `curl` a real product's OG image URL (extract the hashed URL from that product page's `<meta property="og:image">`, same technique used to verify this locally) and confirm 200, not 500. If it 500s, add `outputFileTracingIncludes: { "/products/[slug]/opengraph-image*": ["./public/images/og-logo-ghost.png"] }` (or the equivalent glob) to `next.config.mjs`. (Med value, Low effort) `[relates-to: TASK-057 gate revisions]`
