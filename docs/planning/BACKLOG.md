...
not built. The rest surfaced while applying the malformed-JSON fix across all public POST routes.
All Claude-surfaced → 🟤.
...
- [ ] 🟤 **`api/checkout/create-payment-intent` is the one public JSON route with no malformed-body guard** — G20 applied `request.json().catch(() => null)` to all 8 live public POST handlers and deliberately excluded this one, because the Stripe payment-intent path has been dormant since G2 (2026-08-06) and touching it would imply otherwise. The exclusion is encoded in `tests/unit/malformed-json-body.test.ts`'s `EXCLUDED` list with that reason attached, so the enumeration guard skips it rather than silently passing. Whenever TASK-048 revives payments, this route needs the idiom and its `EXCLUDED` entry removed in the same change. (Low value, Low effort) `[relates-to: TASK-048, G2]`
