...
- 2026-08-14 — **T4–T11 execution record (controller, closes the final review's Important #1)**:
  - T4 site domains complete (`4854d2d`, review Approved): header/footer/home/brand/site namespaces; deviations accepted with evidence — BRAND_TAGLINE kept (opengraph-image.tsx imports it), async HomePage delegates t() to non-async helpers, announcement whole-text-link mode narrowed away (documented).
  - T5 form domains complete (`8d04973`+`13c7297`, review Approved): 6 namespaces incl. account status maps (temp duplicate for admin until G13, order-status.ts unchanged); byCode mechanism = `t.has(key as never)` uniform across 6 spots; shipping.ts map retained for the email/legacy path.
  - T6 products sweep complete (`c755042`, review Approved): products namespace, `getProductBadge()` key-return helper; product-display.ts + opengraph-image.tsx verified correctly unchanged (DB-value swatch keys / formatPrice-comment only).
  - T7 reviews/categories/share complete (`e10336a`+`52b9235`+`cf4ef82`, 1 fix round): full-sweep gate reached; one comment reword corrected back to referencing the catalog key after review caught a false locator quote.
  - T8 SEO fixed set complete (`e022b70`+`1d2e3fe`, 1 fix round): 6 seo helpers async via getTranslations, root layout → generateMetadata; ruling — "Product Not Found" joined the ruled page-titles class (`seo.productNotFound` «Товар не знайдено»); BRAND_DESCRIPTION → catalog (byte-identical, 139/139 chars), BRAND_META_SUFFIX stays (build-time OG route).
  - T9 RU draft complete (`f31f399`, review Approved): 474/474 keys, hard coverage + ICU-arg-parity guards, «Нова Пошта» undeclined-brand convention + gender-resolution choices flagged in messages/README.md (DRAFT — client sign-off rides TASK-056); carried T5 ruling shipped — confirmation page shipping label now catalog-first with legacy fallback.
  - T10 §7.4 axis tests complete (`fe2dd0c`+`4631b29`, 1 fix round — expectations restored to ` ` escape form). **§7.4 verification record (spec §5)**: axis tests green (NBSP grouping asserted by code point, comma decimals, «грн» NBSP-joined suffix, no trailing period, no «,00» on integers, 999 vs 1 000 boundary); `Intl.NumberFormat("uk-UA",{style:"currency",currency:"UAH"})` comparison output: `["1 290,00 ₴","1 290,50 ₴"]` — same separators, `₴` vs the sanctioned «грн» (§7.4 allows either), forced `,00` vs documented ruling 1. formatPrice unchanged, locale-invariant.
  - T11 cleanup/docs/verification complete (`8cfec24`, review Approved): pluralizeUk comment updated (category-client is the surviving consumer until G12); CLAUDE.md ×3 propagated (content tree, cookie-mode i18n pattern, async-SEO rule, superseded byCode notes).
  - **Final verification battery (T11)**: typecheck/lint/format:check clean; unit **776 passed | 1 todo** (from 743 pre-branch); `next build` 53/53 pages (standalone + plugin wrap); byte-diff verifier clean vs main; E2E chromium 29/30 — the 1 failure is the T6-evidenced pre-existing products flake (reproduced on pre-branch commits; CI is the arbiter). Sweep-gate final definition: exactly 4 residual Cyrillic files, all DB-value matchers or comment-only (filter-bar COLORS, product-display swatch keys, SocialLinks comment, seo.ts:240 comment) — zero rendered copy outside the ruled scope.
  - **Final whole-branch review (most capable model)**: merge-ready with this docs commit as the only pre-merge fix; all task-review minors triaged RIDE → close-out BACKLOG batch (duplicate-value sync-test idea for G13, alternates.languages EN corner joins the machine-metadata BACKLOG entry, seo.breadcrumb/products.breadcrumbHome consolidation, AnnouncementBar comment+scope polish, renderWithIntl count = 23 test files).

- 2026-08-15 — **Gate round 2 (user feedback on round 1: "most changes look good" + 1 defect + 4 scope questions)**:
  - Defect: mobile menu sheet unscrollable, sign-out button clipped below the fold. Root cause pre-existing on `main` (the Header's `SheetContent` instance had no `overflow-y-auto`); aggravated in-branch by the added LocaleSwitcher row. Fixed at the usage site (`ui/sheet.tsx` untouched per module convention). Chromium-verified at 375×667: content 685px > viewport 667px, sheet scrolls, sign-out fully visible at scroll end.
  - User ruling (their Q4): order dates should follow the active locale rather than fall back to numeric. Exactly two customer-facing sites hardcoded `toLocaleDateString("uk-UA")` months — account orders list (short month) and order detail (long month + time) — both now use next-intl `useFormatter` (`format.dateTime`), so months render «лип.»/«липня» ↔ «июл.»/«июля» per locale. Deliberate side effect: display timezone becomes the provider's Europe/Kyiv instead of browser-local. Review/testimonial dates are numeric (identical in both locales) and stay; admin dates stay en-US until G13.
  - Verification: typecheck/eslint/byte-diff clean, unit suite 776 | 1 todo (unchanged), scripted chromium checks green both directions (no cross-locale month leakage), gate artifact republished (same URL, round-2 section). Bonus evidence for their Q5: tab title «Увійти | Mirox Shop» ↔ «Войти | Mirox Shop» per cookie — human-visible metadata already localizes via the catalog; crawlers (cookieless) get UA, unchanged.
  - Scope answers queued for close-out BACKLOG (user-raised → 🔵): DB-content localization decision (client conversation, TASK-056 tie), transactional-email localization (persist locale on Order/Subscriber; workers lack request context), Zod validation-message localization (byCode-style, G13-era).

- 2026-08-15 — **PR #37 review round 1** (3 issues, all verified real; fixes `10ad311` + `df67e45`, suite 776→778 | 1 todo):
  - Issue 1 _(fixed differently)_: 4 unguarded dynamic status-key renders on the account pages → `t.has` guards with raw-status fallback (parity with main's `?? status`), plus a Prisma-enum drift net in `i18n-catalogs.test.ts` against `messages/uk.json` (2 new tests; RU transitively covered by the full-parity guard). Deviation from the reviewer's "repoint content.test.ts" suggestion: that block stays — it guards the surviving admin-side `content/account.ts` duplicate and dies with it at G13.
  - Issue 2 _(fixed)_: SocialLinks `formatFollowers` comment promised TASK-039 would localize compact suffixes — falsified by this PR's own recorded scope cut; comment now states the shipped reality.
  - Issue 3 _(fixed)_: five API-route comments cited deleted `src/content/{newsletter,feedback,auth}.ts` as the code→copy target → repointed to each route's `byCode` namespace in `messages/{uk,ru}.json`. Same propagation class as the in-branch-decisions lesson: the routes were unmodified, the deletion falsified them.

- 2026-08-15 — **PR #37 review round 2** (round-1 fixes independently verified by the reviewer incl. mutation-testing the drift net; 1 new finding, fixed):
  - Finding _(fixed — delete path)_: my round-1 justification for keeping `content.test.ts` ("guards the admin-side duplicate") was **false** — the admin panel never consumed the label maps (verified: both admin pages import only `getOrderStatusStyle` and render `{order.status}`/`{order.paymentStatus}` raw; `main`'s single `PAYMENT_STATUS_LABELS` consumer was the account-detail line this PR migrated to the catalog). Deleted `src/content/account.ts`, `getOrderStatusLabel()` + the label re-export (order-status.ts is styles-only now), and the 4 label-only tests; corrected the false claim in root CLAUDE.md (content tree, order-status bullet, Reserved-namespace paragraph) and src/app/CLAUDE.md. Chose deletion over keep-and-correct because the UA copy already lives byte-identical in `uk.json` — G13 loses nothing and sources from the catalog.
  - Adjacent doc defect found while correcting _(fixed)_: root CLAUDE.md claimed the admin panel is "hardcoded-UA" — it is hardcoded **English** (zero Cyrillic in admin/orders/page.tsx); the G13 gap is now recorded accurately in both CLAUDE.md files (EN chrome + raw enum badges until G13 sources labels from the catalog).
  - Suite arithmetic reconciled: 778 → **773 | 1 todo** (−4 deleted label tests, −1 `no-bright-colors` dynamic per-file test for the deleted module — the T5 `−4 dynamic` precedent). Typecheck/lint/byte-diff clean (deleted labels exist verbatim in the catalog).

- 2026-08-15 — **PR #37 review round 3** (round-2 fix verified by the reviewer; 1 residue + 1 note, both fixed):
  - Residue _(fixed)_: the round-1 drift-net header in `i18n-catalogs.test.ts` still carried the falsified "admin-side duplicate / deleted at G13" claims — the round-2 correction swept every file except the one written earliest, exactly root CLAUDE.md's propagation-check failure mode. Header now states the round-2 reality (sole net; module deleted; admin raw until G13).
  - Note _(fixed)_: "hardcoded English chrome" softened in root CLAUDE.md — `ProductForm.tsx` carries three literal «грн» input suffixes (pre-existing), now named so G13 planning doesn't trust the claim as an exhaustive sweep.

- 2026-08-15 — **PR #37 review round 4** (no new code issues; every round 1–3 finding ruled fixed/accepted; PR mergeStateStatus CLEAN):
  - Re-raise _(fixed)_: the SheetOverlay/aria-describedby deferral was accepted on a recording that did not exist — "queued as a 🔵 BACKLOG entry" lived only in chat and the PR thread. Filed now on-branch: BACKLOG `### [2026-08-15] From: PR #37 review chat`, 🔵, with `[possible-dup-of: 🟤 Textarea drops refs, 2026-08-08]` per intake rules (shared root cause: React-19-era shadcn plain-function primitives on React 18). The other two chat deferrals need no entry per the reviewer's ruling (extractor warning has no available action; E2E flake answered by three green CI runs).
