...
| 1a  | ↳ R2 image host swap (G16 shipped)      | `AWS_CLOUDFRONT_URL` (Vercel prod) → `https://img.<domain>` bound to the R2 bucket → redeploy; interim is the rate-limited `r2.dev` URL | 📨     | rides item 1; pre-authorized chain. **G16 dependency cleared 2026-09-01** — the bucket is live and prod images already serve from `pub-444210ee….r2.dev`, so this is now a one-variable swap plus a redeploy, blocked only on the domain |
