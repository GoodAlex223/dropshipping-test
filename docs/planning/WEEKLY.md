...
- [x] Non-Claude AI best-practices: same, for non-Claude tools — carries the bias-watch counter (run 2 of the 2-more-runs condition) (1) — GitHub `spec-kit`: **pass** as a toolchain (its pipeline duplicates the superpowers flow), but `/speckit.analyze` and `/speckit.converge` routed as **design input to G11**, incl. a spec-vs-code direction the linter entry did not cover. **Counter did NOT increment — still 1 of 2**: run 1's condition is "the same reason", and this pass has a different one. The park's condition was rewritten to name the real dead end (searching this category for _portability_ practices, not the category itself).
