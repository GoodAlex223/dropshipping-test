...
**Merge**: `9fc4fd3` (2026-08-18, `--no-ff`, merged locally from `feat/g12-categories-to-catalog` and pushed to `origin/main`; branch deleted, local only — it was never pushed, so there was no remote ref to delete). **No PR** — reviewed as a local branch instead: one round, 4 findings, all cleanup/drift, all fixed. **No commit count is recorded here, deliberately**: four figures were asserted across this task (6, 7, 8, and a corrected 11) against a true 10 branch commits / 12 including the merge, each defensible under an unstated predicate — and the commit written to correct the count incremented it. Use `git rev-list --count d13c8a1..9fc4fd3` or `git log d13c8a1..9fc4fd3`.
...
**Verification**: `typecheck` / `lint` / `format:check` / `test:run` / `build` all exit 0 — the first four re-run **without pipes** after `tail` was found masking exit codes (`format:check` had genuinely failed and been reported clean). Browser gate on a cleared `.next`, screenshots delivered as an Artifact for user sign-off. **E2E resolved after merge**: it was not run on the branch (the `navigation.spec.ts` edit was reasoned and typechecked only, and was flagged as this branch's one unverified change through every report), and CI's first execution of it passed — run `32126515120`, job **E2E Tests**, `60 passed`, which is the full suite (30 tests × chromium+webkit), so the inverted desktop-«Категорії» assertion genuinely executed rather than being skipped. **Production deploy verified by the user** (2026-08-18) — the Vercel Git integration on the `main` push; no `prisma/` changes, so `prisma migrate deploy` was a no-op.
...
### [2026-09-12] G21 - Weekly Reviews: Run 3 (WEEKLY batch, ⚪ Overhead)
...
**PR**: [#47](https://github.com/GoodAlex223/dropshipping-test/pull/47) — merged `05edce3` (2026-09-13, `--merge`). Docs-only, nothing to verify in a running app; `docs-freshness` 109 passed and `prettier --check "docs/**/*.md"` clean before opening and again on every review round
...
- Skeleton-first again (recipe step 2): the run-3 rows and the declared scan windows were committed empty in `527b612` before the first `WebSearch`, with every placeholder carrying no value — run 2's stale-skeleton defect was a pre-filled counter, so this run pre-filled nothing.
- Slot-4 window **widened by user ruling at the brainstorm**, the same call as at G10 and for the same reason: WEEKLY's list was written 2026-08-20, before G16–G20 shipped, and the slot's own definition ("shipped since run 2") governs. Scanned PRs #40–#46, the G11/G12/G15 bare merges, and every memory file touched 2026-08-17 → 2026-09-12.
- **A park closed itself.** `security-guidance` was a run 2 `defer` on per-turn cost; Convention 2's exclusion-set check found it already `true` in the live `~/.claude/settings.json` (v2.0.8 cached). Adopted by another route, so no verdict was owed and its source was never re-read. Recorded as a process row. Design consequence, now in spec §12: run Convention 2 **before** a park re-review, not after.
- **A fired condition still deferred, and its condition was rewritten.** `logic-lens` fired at 24 threshold recurrences — G17 was the first with scores on the record (6×75 + 1×65 against an 80 gate, all seven real) — but the candidate is exactly where run 2 left it (v0.6.10, 22 stars, single maintainer, self-published benchmark). Meanwhile the re-check found the plugin's own README documents changing the gate, and the BACKLOG already names the mechanism: the rubric emits only 0/25/50/75/100, so an 80 gate is a 100 gate. A second reviewer routes around a quantization defect instead of fixing it. Re-trigger rewritten so recurrence count alone no longer fires it.
- **The adopt is a correction to this batch's own rules.** Slot 2's candidate (["How and when to use subagents in Claude Code"](https://claude.com/blog/subagents-in-claude-code), 2026-04-07) supplies the trigger Convention 8 lacks, and checking it exposed the convention overstating its evidence: it bans fan-out because it "gets OOM-killed in this devcontainer", but background Workflow fan-out at width is the only regime that ever did (TASK-038b, `oom_kill 1`, 8.45 GiB of 9.7), while bounded foreground dispatch measured clean (G17: 38 agents, +550 MiB, `oom_kill 0`). Filed 🟤 to amend the wording, carrying both standing constraints into it — ask before any multi-agent review, never re-launch after a session-limit kill.
- Slot 2 also recorded a negative worth keeping: **date-aware search found nothing first-party published since run 2's cutoff.** The whole field was evergreen posts (2026-04-07, 2026-04-15, 2026-05-14).
- Slot 3 searched **methodology, not portability**, per the rewritten bias-watch condition, and deferred EARS (vendor-neutral, Rolls-Royce via IEEE RE09, adopted by Kiro) for a reason worth recording: it formalizes writing acceptance criteria — the half this repo already derives from the PR #18 refinement — and Kiro's docs stop short of proving a criterion can fail, which is the half every recurrence lives in. Counter **does not increment, stays 0 of 2**: a defer that routes a design input is not a barren slot. Third run running without advancing.
- **Slot 4's best find was a half already global.** `guards-need-teeth` is present twice (`CLAUDE.md:41`, `POLICIES/critical-thinking.md:77`) as a principle, while `POLICIES/testing.md` has no technique for building a guard that can go red — greps for `vacuous`, `toBeGreaterThan`, `independently`, `count` all return zero. Propagated the technique half only, with the empty-directory ENOENT and fixture-vacuity cases as its worked examples.
- Routing: 1 🟤 for the adopt + 1 🟤 incidental (Convention 5), 1 design-input fold-in on the live rubric entry, 3 `TODO.md` § 🔀 Spawned rows (the queue now stands at **9**, all unchecked by design), and 6 slot-4 defers deliberately routed as **fold-ins** against hosts that already have queued edits rather than as new errands — 5 new, plus "claimed records must exist" re-parked into the fold run 2 already opened for it, now carrying its 2026-09-02 update.

**Step-5 re-check pass** — caught one real defect on its second outing as a formal step. Check 4 (attribution inside the declared window) found the grouped stack-facts row citing **PRs #40, #44, #46**; verified against the memory files themselves, only #44 and #46 were origins at all, and the correct set is the G11 and G12 merges plus PRs #41/#43, #44 and #46. Every other Origin cell was checked the same way and held. Check 1 cleared a false positive before anything was "fixed": a naive count read **11** conventions in `REVIEW-QUEUE.md` against the spec's **17**, both artefacts of unrelated numbered lists — enumerated, both files carry exactly **10**, aligned 1:1. Check 5 refreshed Convention 1's cadence with run 3's figure and named the slot that is actually growing.

**PR review: 4 rounds; both findings in the same paragraph.** r1 — 1 finding: after the self-review corrected the fold-in count 5 → 6, the bullet's trailing clause still read "five edits against four files". Fixed **differently** rather than by the bump, and re-deriving both figures showed the clause had **never** been right: at five items the hosts were five files with **three** queued rows, not four. The reviewer independently reproduced that and endorsed declining the straight bump. r2 — 1 finding: the sentence added for r1's second point ("the sixth adds no host to that tally") contradicted the six-file list five lines above it, since `documentation.md` is in that list _only_ because of the sixth item. A referent problem, not an arithmetic one — fixed by naming what the sixth actually does not open: a new **fold**, not a new host. r3 and r4 — no issues in the change; the clause held a third and a fourth independent re-derivation, and the 17-row tally, the 3/1/2/11 breakdown, the 9/14/17 and 5→8→11 sequences and the 13 park rows were never touched by any fix. r4 corrected its own r3 figure unprompted (`[[…]]` occurrences across `docs/`: **19**, not 20 — it had counted lines, and one line was a JS array literal) and then ruled explicitly on all six earlier non-`fixed` verdicts rather than letting silence close them, accepting four and re-raising two for the record: the gate instance and the routing of the r2 mechanism, both below.

**The r2 mechanism is worth more than the r2 defect** (reviewer's own framing, and it generalizes): _repairing two review points inside the same paragraph is what made the two repairs disagree_. Each fix was correct in isolation; the second was written against the paragraph as it read before the first landed. This is the [[review-fix-chains-and-lazy-diagnostics]] shape — a fix moving a defect downstream — narrowed to its tightest case, where downstream is four lines away in the same bullet. The cheap control is to re-read the whole unit after the last edit rather than after each one.

**Round 1 was gate-suppressed — the instance the 🟤 rubric row never had.** The reviewer reports the r1 finding scoring **75** against the plugin's **80** gate: under the procedure no comment would have been posted, and it reached this PR only because the maintainer directed the review. It then proved not merely stale but wrong on _both_ figures — understated by the gate in exactly the direction the pattern predicts. That is recurrence **25** of `code-review-threshold-understates-doc-findings`, and the first where a suppressed finding is on the record with its score _beside the defect it actually found_; noted on the 🟤 rubric entry, which until now named the mechanism with no instance attached. The score is the reviewer's own instrument reading and is recorded as such — this repo cannot re-measure it. It does **not** fire the `logic-lens` park: that re-trigger was narrowed by this same run so recurrence count alone no longer counts as evidence, and 25 was named in advance as insufficient.

**The r2 mechanism is now routed — but not as a seventh fold-in.** Round 4's fair complaint was that a lesson called "the one thing I would carry to another project" had no sink: `grep -rn "fix-chain" docs/planning/` returned this entry and nothing else. The cause was upstream of routing — the lesson lived only in `DONE.md` prose, and slot 4 scans **memory files**, which is what all six fold-in rows cite. So the source was fixed: it is now the fourth failure mode in `[[review-fix-chains-and-lazy-diagnostics]]`, with the control stated, and its absence from the global tree is pre-verified and recorded in `REVIEW-QUEUE.md` so next run's slot 4 gives it a verdict instead of re-deriving it. Filing it as a run-3 slot-4 row was declined on a stronger ground than the cost of reopening the six/four clause: those rows record what a scan found before this PR existed, and each cites a memory file that already held its lesson then — a row for a lesson written after the scan would be a claimed record that does not match what happened, which is the failure mode this run filed a row about.

**Close-out extraction (2026-09-13)**: 2 🟤 filed from the review arc itself, both about the loop rather than the run — step 5's count check does not cover figures **derived** inside a sentence (which is where both PR findings were), and a `fixed differently` / `pushed back` / `recorded, not fixed` verdict has no adjudication step, so six of them sat unruled until round 4 volunteered rulings. Nothing to archive (no plan). No deploy-tied work spawned: the branch is docs-only, and the open 🟠 stale-CSS carry predates it.

**Learnings**: the parks are now doing more work than discovery is. Two of this run's three most useful results came from re-checking conditions set weeks ago — one park resolved by adoption elsewhere, one fired and still said no — and the exclusion set is what detected the first. A condition that keeps firing while its answer keeps being "no" is mis-specified, not vindicated; rewriting it is part of respecting it.
