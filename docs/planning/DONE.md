...
- Routing: 1 🟤 for the adopt + 1 🟤 incidental (Convention 5), 1 design-input fold-in on the live rubric entry, 3 `TODO.md` § 🔀 Spawned rows (the queue now stands at **9**, all unchecked by design), and 5 slot-4 defers deliberately routed as **fold-ins** against hosts that already have queued edits rather than as new errands.
