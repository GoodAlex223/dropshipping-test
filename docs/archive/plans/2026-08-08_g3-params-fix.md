...
- 2026-08-08: PR #30 review round 2 — no code issues on `00c73d6`; CI fully green; merge state CLEAN. Two BACKLOG-wording refinements, both verified and applied: (1) docs-freshness linter entry gains the recurrences #8/#9 update line (promotion evidence must live in the entry, not the PR thread); (2) Textarea entry rewritten with confirmed cause (plain function component vs `Input`'s `forwardRef`; RHF ref dropped for `shortDesc`/`description` → no focus/scroll on validation error), Med/Low priority, React-19-retires-it note. Pushed back on one round-2 narrative claim: the old `global.fetch` stub could not leak across test files — `vitest.config.ts` uses default per-file isolation; hardening stands as future-proofing.
