...
> **How to read the evidence in this document.** Every claim was researched and then **adversarially re-checked against its primary source** by a second pass instructed to refute it. **120 claims, all 120 verified** — outcome per claim: **77 confirmed, 33 disputed, 10 unverifiable**. (Those 120 claims drew **145 verdicts**, because 16 claims were re-checked more than once during crash/resume cycles; a claim is counted as disputed if _any_ verdict disputed it.) Markers used throughout:
>
...
**Status of the evidence:** 120 claims researched, **all 120 verified** against primary sources — **77 confirmed, 33 disputed, 10 unverifiable**. A **27.5% dispute rate** (33/120) on the raw research is the reason this document exists in this form; unverifiable figures are tagged, never guessed.
...
- Cost: **1.3–2%** of order value — every figure and its source is in §4 (LiqPay and Plata by mono 1.3% domestic / 2% foreign; WayForPay and Portmone 2%).
- Settlement: UAH, next banking day for every viable candidate (§4).
...
- Cost — **two different products, two different tariffs.** Getting these confused is the most common modelling error here:
  - **COD → sender's card** (the retail default): **1% + 10 UAH** to a NovaPay card (money in ~30 min), **2% + 20 UAH** to any other card or in cash (next business day). Behaviourally confirmed against the live API's `CostRedelivery`, which matched `0.02·N + 20` exactly across all tested amounts. [novaposhta.ua/financial-services/money-transfer/to-sender](https://novaposhta.ua/financial-services/money-transfer/to-sender/) + live API, verified 2026-07-17.
  - **COD → business account** (NovaPay «Зарахування післяплати на рахунок», the merchant-grade route): _Universal_ tariff — **0.5%** credited to a NovaPay account **same day**, **1.3%** to any other bank **next business day**. _Standard_ tariff (recipient pays the whole fee) — **2.5%** / **3.3%**. [novapay.ua/pisljaplata-na-rahunok](https://novapay.ua/pisljaplata-na-rahunok/), verified 2026-07-17.
- Ceiling: **399,999 UAH** per single післяплата (both routes) — [novaposhta.ua/financial-services/money-transfer/to-sender](https://novaposhta.ua/financial-services/money-transfer/to-sender/), verified 2026-07-17.
...
**Field coverage:** all **16** fields mandated by the spec are present, shown as **17 rows** — mandated field 1, "Card fee (% + fixed)", is split into _Card fee_ and _Fixed fee_ for readability (every candidate is percentage-only, which is itself worth seeing at a glance). Row count is therefore not a proxy for coverage; the field list is.
...
| **Support**          | ⚠️ Email, phone 3700, **24/7 online chat**; UA    | Email, phone, Viber/Telegram bot, KB; UA/EN docs ✅ | monoбізнес portal + Telegram/Viber; **UA only, no EN merchant support** ✅ | Phone, B2B Telegram/Viber bot; **24/7**; UA ✅ | ❓ (Ukraine-side unreachable)         |
...
**COD limits:** **399,999 UAH** per single післяплата — the per-shipment ceiling, stated on the primary source. Do not confuse it with the **card-payout** limits (**100,000 UAH** to a NovaPay card; **29,999 UAH** to other Ukrainian bank cards; **~349,999 UAH** rolling monthly), which cap the _payout_, not the shipment. Both sets of figures: [novaposhta.ua/financial-services/money-transfer/to-sender](https://novaposhta.ua/financial-services/money-transfer/to-sender/), verified 2026-07-17.
