...
- **UI primitives**: `ui/` directory contains shadcn/ui components — regenerate with CLI rather than editing directly. Two sanctioned exception classes, both of which must be re-applied after any CLI regeneration: (1) `ui/sonner.tsx` is hand-edited (TASK-034 dropped `next-themes`, so its `useTheme()` had to go and `theme` is hardcoded to `"light"`) — regeneration would otherwise reintroduce the import with no provider mounted; (2) `ui/button.tsx`, `ui/input.tsx` and `ui/textarea.tsx` are wrapped in `React.forwardRef` with an explicit `displayName` — react-hook-form's `register()` returns a `ref` that must reach the underlying DOM node, and a plain function component drops it silently, so the field registers but never reads or validates. `button.tsx` gained it in `ecb197b` for dropdown compatibility, `textarea.tsx` in G16 for `ProductForm`'s description fields
...
- **Image optimization**: `ProductImage` (rendered by `ProductCard`) uses `DEFAULT_BLUR_DATA_URL` and the `IMAGE_SIZES.productCard` sizes passed by `ProductCard` from `@/lib/image-utils` for optimized loading with blur placeholders; on a missing/broken `src` it renders the branded Mirox "M" fallback instead of a blank box
